import pandas as pd
from sqlalchemy import create_engine, text

# Lee el archivo Excel que está en el directorio 'data' dentro del proyecto
archivo_excel = '../data/Internet.xlsx'

# 'sheet_name=3' lee la cuarta hoja (índice basado en 0)
df_hoja4 = pd.read_excel(archivo_excel, sheet_name=3)

# Crear una conexión a la base de datos SQL
engine = create_engine('mysql+pymysql://root:adm123@localhost:3306/internet')

# Consultar la tabla 'provincia' para obtener 'id' y 'provincia'
with engine.connect() as conn:
    query = text("SELECT id, provincia FROM provincia")
    df_provincia_sql = pd.read_sql(query, conn)

# Realizar un merge con df_hoja4 para obtener la columna 'fk_provincia'
df_merged = df_hoja4.merge(df_provincia_sql, left_on='Provincia', right_on='provincia', how='left')
df_merged = df_merged.rename(columns={'id': 'fk_provincia'})

# Verificar el contenido del DataFrame después del merge
print("Contenido de df_merged:")
print(df_merged.head())

# Crear la tabla 'vmd_provincia' si no existe
with engine.connect() as conn:
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS vmd_provincia (
            id INT AUTO_INCREMENT PRIMARY KEY,
            anio INT NOT NULL,
            trimestre INT NOT NULL,
            fk_provincia INT NOT NULL,
            velocidad_media DOUBLE NOT NULL,
            FOREIGN KEY (fk_provincia) REFERENCES provincia(id),
            INDEX idx_fk_provincia (fk_provincia),
            INDEX idx_anio (anio),
            INDEX idx_trimestre (trimestre)
        )
    """))

# Eliminar las columnas 'Provincia', 'provincia'
df_merged = df_merged.drop(columns=['Provincia', 'provincia'])

# Renombrar las columnas para que coincidan con la tabla de MySQL
df_merged = df_merged.rename(columns={
    'Año': 'anio',
    'Trimestre': 'trimestre',
    'Mbps (Media de bajada)': 'velocidad_media'
})

# Cargar los datos en la tabla de MySQL
df_merged.to_sql('vmd_provincia', con=engine, if_exists='append', index=False)

print("Datos cargados exitosamente en la base de datos.")
